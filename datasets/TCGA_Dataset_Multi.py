...
class TCGA_Dataset(data.Dataset):
    def __init__(self, excel_file, modal, signatures=None, data_root_wsi=None, data_root_omics=None):
        ...
        #
        if "Omics" in self.modal:
            self.omics_size, _ = self.read_Omics(self.rows["Omics"].dropna().values.tolist()[0])
        ...
        if "WSI" in self.modal:
            self.path_size, _ = self.read_WSI(self.rows["WSI"].dropna().values.tolist()[0])
    ...
    def read_WSI(self, wsi):
        ...
        if wsi != "":
            ...
            wsi = torch.zeros(1, self.path_size).type(torch.float32)
            return wsi, False
    ...
    def read_Omics(self, omics):
        ...
        if omics != "":
            ...
            if self.signatures is None:
                # get Gene and Value columns
                genes = df["Gene"].values.tolist()
                values = df["Value"].values.tolist()
                values = torch.from_numpy(np.array(values)).type(torch.float32)
                omics = (genes, values)
            else:
                omics = []
            ...
            omics = [torch.zeros(size).type(torch.float32) for size in self.omics_size]
            return omics, False
