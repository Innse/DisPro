...
class Engine(object):
    ...
    def learning(self, model, dataloaders, criterion, optimizer, scheduler):
        ...
        for epoch in range(self.args.num_epoch):
            ...
            print(">>>")
            print(">>>")
        self.results["omics"]["c-index"] = 0.0
        self.results["omics"]["epoch"] = 0
        return self.results
    ...
    def train(self, data_loader, model, criterion, optimizer):
        ...
        for batch_idx, (data_Study, data_ID, data_WSI, WSI_Flag, data_Omics, Omics_Flag, data_Event, data_Censorship, data_Label) in enumerate(dataloader):
            ...
            for i in range(len(data_Omics)):
                inputs["x_omic%d" % (i + 1)] = data_Omics[i]
            if torch.cuda.is_available():
                for k, v in inputs.items():
                    inputs[k] = v.cuda()
            # prediction
            logits = model(x=inputs, c=c)
    ...
    def save_checkpoint(self, state):
        ...
        self.filename_best = os.path.join(self.results_dir, "fold_" + str(self.fold), "model_best_{epoch}.pth.tar".format(epoch=state["epoch"]))
