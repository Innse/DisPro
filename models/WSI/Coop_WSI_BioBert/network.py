...
from .model_utils import load_pretrained_bert, tokenize, \
...
class PromptLearner(nn.Module):
    def __init__(self, tokenizer, token_embedding, ctx_dim, prompt_config, dtype=None):
        ...
        tokenized_prompts_ids, tokenized_prompts_attn = tokenize(tokenizer, prompts, max_len_cls) # ids for classname description
...
class CustomCLIP(nn.Module):
    def __init__(self, tokenizer, clsStr_encoder, prompt_config, modal_config):
        ...
        if modal_config.modal_enc_name == 'fc':
            from .model_utils import WSI_head as modal_encoder
    ...
    def forward(self, x, c):
        ...
        if self.modal_config.pool_method == 'topj':
            logits = topj_pooling(logits, self.modal_config.topj)
